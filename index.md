...
- HEADが指すブランチの最新コミットの子に新たなコミットを追加するコマンド
...
- ブランチ: そのブランチ（開発ライン）における最新のコミットを指すポインタ。
- HEAD: 「いま自分がどのブランチにいるか」を指すポインタ。
 
通常、作業中は HEADがブランチを指し、そのブランチが最新コミットを指す という二段階のポインタ構造になっています。 
...
- ポインタが削除されるだけでブランチが指していたコミットは残る
...
git merge トピックブランチ
```
- HEADが指すブランチに最新コミットを追加し、指定したブランチ（ここではトピックブランチ）のコミットを取り込むコマンド
...
2. `--mixed`（省略可） : 1.の後、インデックスの内容をHEADと同じにする
3. `--hard` : 2.の後、作業ディレクトリの内容をインデックスと同じにする
